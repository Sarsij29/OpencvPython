...
frameWidth = 640
frameHeight = 480
nPlateCascade = cv2.CascadeClassifier("Resouces/haarcascade_russian_plate_number.xml")
minArea=500
color=(255,0,255)
###############################
...
while True:
    ...
    for (x, y, w, h) in numberPlates:
        ...
        if area>minArea:
            ...
            cv2.imshow("NumPlate",imgNumPlate)
    cv2.imshow("Result",img)

    if cv2.waitKey(1) & 0xFF == ord('s'):
        cv2.imwrite("Resouces/Scanned NumPlates/NoPlate_"+str(count)+".jpg",imgNumPlate)
